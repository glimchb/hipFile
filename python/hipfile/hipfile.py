...
# ---------------------------------------------------------------------------
# Initialise library  (inject into bindings module, mirrors cufile pattern)
# ---------------------------------------------------------------------------

def _get_lib() -> None:
    if _bindings._lib is None:
        lib = _load_library()
        _setup_functions(lib)
        _bindings._lib = lib
    return _bindings._lib
...
# ---------------------------------------------------------------------------
# Buffer registration helpers  (mirrors cufile.bindings exports)
# ---------------------------------------------------------------------------

def buf_register(buf, size: int, flags: int = 0) -> None:
    """Register a GPU buffer for hipFile I/O."""
    _get_lib()
    if isinstance(buf, int):
        buf = ctypes.c_void_p(buf)
    hipFileBufRegister(buf, size, flags)
